...
class frameDataset3(VisionDataset):
    def __init__(self, base, train=True, transform=ToTensor(), target_transform=ToTensor(),
        ...
        checkgp = self.gt_fpath.find('Terrace')
        if not os.path.exists(self.gt_fpath) or force_download or checkgp is not True:
            self.prepare_gt()
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None and self.trainstat is True:
            random_list, bbox_by_pos_cam = generate_occlusion(self.base, 0, 1320, 20)
